...
! ##############################################################################
  ...
  subroutine axes_init(      deg_lat,                         deg_lon,        &
                             sin_lat,                                         &
...
! ------------------------------ local variables ------------------------------
    ...
    character(len = nf90_max_name) :: lat_name, lon_name
    integer :: i, k, latVarID, lonVarID
  real, dimension(160) ::                                                     &
...
! ------------------------------ executable code ------------------------------
    ...
    if(data_source .eq. era40) then
       ...
       call check(nf90_Inquire_Dimension(TEMPncInputID, dimIDs(1), name=lon_name))
       call check(nf90_Inquire_Dimension(TEMPncInputID, dimIDs(2), name=lat_name))

       ! read in latitude and longitude
       call check(nf90_inq_varid(TEMPncInputID, trim(lon_name),    lonVarID) )
       call check(nf90_get_var  (TEMPncInputID, lonVarID, deg_lon))
