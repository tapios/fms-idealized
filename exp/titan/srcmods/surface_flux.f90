...
! ============================================================================
  ...
  real, intent(inout), dimension(:) :: depth_change  
  real, intent(in) :: dt
  ...
  t_surf0 = 0. !200. !  avoids out-of-bounds in es lookup  ! LJJ: modified based on TS codes 
  where (avail)
     where (land)
        t_surf0 = t_ca
     elsewhere
...
!     thv_surf= t_surf0  ! surface virtual (potential) T -- just for testing tun off the q_surf
  ...
  where (avail)
     ...
     ! begin LJJ addition
     depth_change  = flux_q * dt/dens_h2o 
     where (flux_q > 0.0 .and. bucket_depth < depth_change) ! where more evaporation than what's in bucket, empty bucket
        flux_q = bucket_depth * dens_h2o / dt
        depth_change = bucket_depth
     endwhere 
...
subroutine surface_flux_2d (                                           &
  ...
  do j = 1, size(t_atm,2)
     call surface_flux_1d (                                           &
          ...
          depth_change_lh(:,j),    u_surf(:,j),    v_surf(:,j),             &
