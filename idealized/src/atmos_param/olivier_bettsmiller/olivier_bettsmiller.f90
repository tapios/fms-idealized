...
use      constants_mod, only:  HLv,HLs,Cp_air,Grav,rdgas,rvgas, kappa
...
!   ---- version number ----
 ...
 character(len=128) :: version = '$Id: bm_omp.f90,v 1 2003/05/01 14:50:00 fms Exp $'
...
! added by CW
    real, parameter :: cph2ovapor = 1875.0  ! J/kg/K
...
! code
!             call capecalc( cp, cph2ovapor, d622, kx, pfull(i,j,:),&
...
! moist adaibat derivatives: (use the lcl values for temp, humid, and 
! pressure)
            a = kappa*tlcl2 + hlv/cp*rm
            b = hlv**2.*rm/cp/rvgas/tlcl2**2.
            dtdlnp = a/(1. + b)
! first order in p
...
! first get temp halfway up 
         ...
         a = kappa*tp(klcl2) + hlv/cp*rp(klcl2)
         b = hlv**2./cp/rvgas*rp(klcl2)/tp(klcl2)**2.
         dtdlnp = a/(1. + b)
! second half of RK2
         tp(klcl2) = tlcl2 + dtdlnp*log(p(klcl2)/plcl2)
!            d2tdlnp2 = (kappa + b - 1. - b/tlcl2*(hlv/rvgas/tlcl2 - &
!                          2.)*dtdlnp)/ (1. + b)*dtdlnp - hlv*rm/cp/ &
!                          (1. + b)
! second order in p
!            tp(klcl2) = tlcl2 + dtdlnp*log(p(klcl2)/plcl2) + &
