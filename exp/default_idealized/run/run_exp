...
set machine        = euler                          # machine = euler or brutus, use the alternate runscripts for fram, or change mkmf templates, submission commands, and modules for other machines
set analysis_type  = 2d                             # choose type of analysis: 2d (zonally averaged) or 3d (zonally varying) outputs
set run_name       = RUN_NAME                       # label for run; output dir and working dir are run_name specific
set run_script     = $cwd/run_test                  # path/name of this run script (for resubmit) 
...
set runs_per_script = 28                             # number of runs within this script
...
EOF
	...
	echo "set isegment = 1" >> post_processing_info
	   ...
	   echo "set final = 0" >> post_processing_info
        ...
        #/cluster/apps/lsf/8.0/linux2.6-glibc2.3-x86_64/bin/bsub < $analysis_script
    else
