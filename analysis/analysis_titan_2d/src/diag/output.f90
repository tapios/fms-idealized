...
! #############################################################################
    ...
    call variable_init("u_diff_tend",                                      & ! ljj addition
         "Zonal Wind Tendency due to Subgrid-Scale Diffusion",             &
         "m/s^2",                                                          &
         sigma_switch,                                                     &
         statVarID(185))

    call variable_init("v_diff_tend",                                      & ! ljj addition
         "Meridional Wind Tendency due to Subgrid-Scale Diffusion",        &
         "m/s^2",                                                          &
         sigma_switch,                                                     &
         statVarID(186))
   
    call variable_init("u_diss_tend",                                      & ! ljj addition
         "Zonal Wind Tendency due to Numerical (Spectral) Damping",        &
         "m/s^2",                                                          &
         sigma_switch,                                                     &
         statVarID(187))

    call variable_init("v_diss_tend",                                      & ! ljj addition
         "Meridional Wind Tendency due to Numeridcal(Spectral) Damping",   &
         "m/s^2",                                                          &
         sigma_switch,                                                     &
         statVarID(188))

    call variable_init("temp_diss_tend",                                   &  ! ljj addition
         "Temperature Tendency due to Numerical (Spectral) Damping",       &
         "K/s",                                                            &
         sigma_switch,                                                     &
         statVarID(189))

      
    call variable_init("ke_diss_zonal",                                    &  ! ljj addition
         "Dissipation of Zonal Kinetic Energy from Spectral Damping",      &
         'W/kg',                                                           &
         sigma_switch,                                                     &
         statVarID(190))

    call variable_init("ke_diss_mrdnl",                                    &  ! ljj addition
         "Dissipation of Meridional Kinetic Energy from Spectral Damping", &
         "W/kg",                                                           &
         sigma_switch,                                                     &
         statVarID(191))

    call variable_init("ke_diff_zonal",                                    &  ! ljj addition
         "Dissipation of Zonal Kinetic Energy by Turbulent Mixing",        &
         "W/kg",                                                           &
         sigma_switch,                                                     &
         statVarID(192))

    call variable_init("ke_diff_mrdnl",                                    &  ! ljj addition
         "Dissipation of Meridional Kinetic Energy by Turbulent Mixing",   &
         "W/kg",                                                           &
         sigma_switch,                                                     &
         statVarID(193))
   ...
   call variable_init("eape2eke_flux_psd",                                    &
    ...
    call variable_init("dp_dt",                                               &  ! added by LJJ
         "pressure tendency",                                                 &
         "Pascal/s",                                                          &
          sigma_switch,                                                       &
          statVarID(200))

    call variable_init("conv_pe_ke",                                          &  ! added by LJJ
             "total conversion of potential to kinetic energy",               &
             "m.m/s/s/s",                                                     &
             sigma_switch,                                                    &
             statVarID(201))

     call variable_init("conv_eddy_pe_ke",                                    &  ! added by LJJ
              "eddy conversion of potential to kinetic energy",               &
              "m.m/s/s/s",                                                    &
              sigma_switch,                                                   &
              statVarID(202))
...
!------------------          write coordinate axes         --------------------
    ...
    call check(nf90_put_var(ncFileID, statVarID(185), dt_u_diff_avg,                        &  ! ljj addition
          start = (/ 1, 1 /), count = (/ num_lat, num_lev /) ))

    call check(nf90_put_var(ncFileID, statVarID(186), dt_v_diff_avg,                        &  ! ljj addition
          start = (/ 1, 1 /), count = (/ num_lat, num_lev /) ))

    call check(nf90_put_var(ncFileID, statVarID(187), dt_u_diss_avg,                        &  ! ljj addition
          start = (/ 1, 1 /), count = (/ num_lat, num_lev /) ))

    call check(nf90_put_var(ncFileID, statVarID(188), dt_v_diss_avg,                        &  ! ljj addition
          start = (/ 1, 1 /), count = (/ num_lat, num_lev /) ))

    call check(nf90_put_var(ncFileID, statVarID(189), dt_temp_diss_avg,                     &  ! ljj addition
          start = (/ 1, 1 /), count = (/ num_lat, num_lev /) ))

    call check(nf90_put_var(ncFileID, statVarID(190), ke_diss_zonal_avg,                     &  ! ljj addition
          start = (/ 1, 1 /), count = (/ num_lat, num_lev /) ))

    call check(nf90_put_var(ncFileID, statVarID(191), ke_diss_mrdnl_avg,                     &  ! ljj addition
          start = (/ 1, 1 /), count = (/ num_lat, num_lev /) ))

    call check(nf90_put_var(ncFileID, statVarID(192), ke_diff_zonal_avg,                     &  ! ljj addition
          start = (/ 1, 1 /), count = (/ num_lat, num_lev /) ))

    call check(nf90_put_var(ncFileID, statVarID(193), ke_diff_mrdnl_avg,                     &  ! ljj addition
          start = (/ 1, 1 /), count = (/ num_lat, num_lev /) ))
    ...
    call check(nf90_put_var(ncFileID, statVarID(200), dpdt_avg,                               &  ! added by LJJ
            start = (/ 1, 1 /), count = (/ num_lat, num_lev /) ))

    call check(nf90_put_var(ncFileID, statVarID(201), conv_pe_ke_avg,                         &  ! added by LJJ
            start = (/ 1, 1 /), count = (/ num_lat, num_lev /) ))

    call check(nf90_put_var(ncFileID, statVarID(202), conv_eddy_pe_ke_avg,                    &  ! added by LJJ
            start = (/ 1, 1 /), count = (/ num_lat, num_lev /) ))
