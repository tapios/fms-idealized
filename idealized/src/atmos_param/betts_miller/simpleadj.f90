...
!
  ...
  real :: slope           ! slope of relaxation temperature
...
! register fields with diagnostic manager
    ...
    id_ztsum = register_diag_field ( mod_name, 'ztsum',    &
         axes(1:2), Time, 'ztsum', 'degrees',  &
         missing_value=missing_value)
    id_ztsum1 = register_diag_field ( mod_name, 'ztsum1',   &
         axes(1:2), Time, 'ztsum1', 'degrees',  &
         missing_value=missing_value)
...
!#######################################################################
  ...
  subroutine betts_miller(   is,       ie,       js,       je,              &
    ...
    real, dimension(size(dt_tg,1), size(dt_tg,2)) :: ztsum  !
    real, dimension(size(dt_tg,1), size(dt_tg,2)) :: ztsum1  !
...
! send data to diagnostic manager
    ...
    if(id_ztsum  > 0 ) used = send_data ( id_ztsum , ztsum, Time, is, js)
    if(id_ztsum1  > 0 ) used = send_data ( id_ztsum1 , ztsum1, Time, is, js)
...
!*/
   SUBROUTINE BMADJ(							 &
	 ...
	 ,RSFC,	    SSFC,     LPLAND,	APRC,	  APRS,	    ztc          &
         ...
         ,dztref,   sztref,   ibase,    itop, ztsum, ztsum1)
...
! new scheme for determining stable/unstable:
!     look at potential temperature of current level relative to level below:
!     if larger, stable.  if smaller, unstable.  simple.
      if(jk.lt.nlev) then
         if( ztp1(jl,jk) * xlg((1.e5/app1(jl,jk)),zcons1) .lt.             &
              ztp1(jl,jk+1) * xlg((1.e5/app1(jl,jk+1)),zcons1)) then
            if(ibase(jl).eq.nlev+1) ibase(jl)=jk+1
            itop(jl) = jk
         endif
      endif
!
...
!
      DO 411 JL=1,NLON
      LLOC(JL)=(itop(jl).ne.nlev).or.(ibase(jl).ne.nlev+1)
      IF (LLOC(JL)) THEN
	 ITEST(JL)=1
      ELSE
	 ITEST(JL)=0
...
!
	  ...
	  ,ZDT,	     ZDQ,      ZSC					    &
	  ,IBASE,    ITOP						    &
	  ,IHCTOP,   ILOBAS,   sztref, ztsum, ztsum1                      &
	  ) 
!
...
!*/
	  ...
	  ,PDT,	     PDQ,      PSC					    &
	  ,KBASE,    KTOP						    &
	  ,KHCTOP,   KLOBAS,   sztref, ztsum0, ztsum1                        &
	  )
...
!
      ...
      CALL SCATTER(NSHAL,ztsum,KDX,ztsum0)
      CALL SCATTER(NSHAL,ztsum1,KDX,ztsum1)
!
!     ------------------------------------------------------------------
!
!*	   6.	  RETURN WORKSPACE.
