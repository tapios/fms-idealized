...
! constants for calculation of SVP
  ...
  real, private, allocatable, dimension(:) ::     pk,       bk,       bk_full        &
          ,wts_lat,  sin_lat2
...
! diagnostic IDs
  ...
  integer :: id_dztref, id_top, id_base, id_dztref0, id_dztref1
...
!#######################################################################
  ...
  subroutine betts_miller_init(axes, Time, bk_local, local_sin_lat)
    
    integer, intent(in) :: axes(4)
    type(time_type), intent(in) :: Time

    real, dimension(:), intent(in) :: bk_local
    real, dimension(:,:), intent(in) :: local_sin_lat

    integer :: ierr, namelist_unit, io, num_lat
    
    num_levels = size(bk_local,1) - 1
    lon_max = size(local_sin_lat,1)
    num_lat = size(local_sin_lat,2)

    allocate (        pk(num_levels+1) )
    allocate (	      bk(num_levels+1) )
    allocate (	 bk_full(num_levels  ) )
    allocate (	 wts_lat(num_lat) )
    allocate (	sin_lat2(num_lat) )
...
!*/
   SUBROUTINE BMADJ(							 &
	 ...
	 ,RSFC,	    SSFC,     LPLAND,	APRC,	  APRS,	    ztc          &
         ,dtd,      dqd,      dts,      dqs                              &
         ,dztref0,  dztref1,  dztref,   ibase,    itop     ) 
...
!
!!$      IF (ILAB(JL,JK).EQ.1.or.jk.gt.18) THEN
...
!!$      do jl=1,nlon
!!$         write(*,*) jl,ibase(jl),itop(jl),ilift(jl),itest(jl),              &
!!$              .not.(ZSGTP(JL).GE.ZCTOPS),ZCAPE(JL),                         &
!!$              .not.((ILIFT(JL)-1).EQ.ITOP(JL))
!!$      enddo
!
...
!      ZETPBL=0.9  ! this is the default
   ...
   10 CONTINUE
!
! we do want the boundary layer...
!!$      if(int(alv).eq.0) npbl = 0 ! turn off boundary layer for dry runs
!
! set npbl to zero to turn off boundary layer
      npbl = 0
...
! if not relaxing to surface, comment out the following two lines
...
! don't relax to surface
!!$       if ((icount(jl).eq.0).and.(jk.ge.itopd(jl))) then 
