...
! ---------------------------input file variables--------------------------
  ...
  integer :: UncInputID,     &   ! zonal wind input file id
       ...
       DiabRadncInputID,     &
       DiabSWncInputID,      &
       UMomDiffncInputID,    &   ! ljj addition
       VMomDiffncInputID,    &   ! ljj addition       
       UMomDissncInputID,    &   ! ljj addition
       VMomDissncInputID,    &   ! ljj addition
       DiabDissncInputID,    &   ! ljj addition
       DragMOncInputID,      &
       DragLHncInputID,      &
       DragSHncInputID,      &
       ...
       DiabCondVarID,        &
       DiabConvVarID,        &
       DiabDiffVarID,        &   ! temperature diffusion tendency from the turbulent mixing 
       DiabRadVarID,         &
       DiabSWVarID,          &
       UMomDiffVarID,        &   ! momentum diffusion (U) tendency from the turbulent mixing; ljj addition
       VMomDiffVarID,        &   ! momentum diffusion (V) tendency from the turbulent mixing; ljj addition
       UMomDissVarID,        &   ! momentum dissipation (U) tendency from the spectral damping; ljj addition
       VMomDissVarID,        &   ! momentum dissipation (V) tendency from the spectral damping; ljj addition
       DiabDissVarID,        &   ! temperature dissipation (T) tendency from the spectral damping; ljj addition     
       BucketDepthVarID,     &
...
! ljj addition
       call check(nf90_open(InputFileName, nf90_nowrite, UMomDiffncInputID ) )
       call check(nf90_inq_varid(UMomDiffncInputID, trim(UMomDiffVarName), UMomDiffVarID) )
       call check(nf90_open(InputFileName, nf90_nowrite, VMomDiffncInputID ) )
       call check(nf90_inq_varid(VMomDiffncInputID, trim(VMomDiffVarName), VMomDiffVarID) )
       call check(nf90_open(InputFileName, nf90_nowrite, UMomDissncInputID ) )
       call check(nf90_inq_varid(UMomDissncInputID, trim(UMomDissVarName), UMomDissVarID) )
       call check(nf90_open(InputFileName, nf90_nowrite, VMomDissncInputID ) )
       call check(nf90_inq_varid(VMomDissncInputID, trim(VMomDissVarName), VMomDissVarID) )
       call check(nf90_open(InputFileName, nf90_nowrite, DiabDissncInputID ) )
       call check(nf90_inq_varid(DiabDissncInputID, trim(DiabDissVarName), DiabDissVarID) )
! end ljj addition
...
! ljj addition
      call check(nf90_get_var(UMomDiffncInputID, UMomDiffVarID, dt_u_diff_grid,  &
          start = (/ 1, 1, 1, int(nsteps)/),                               &
          count = (/ local_num_lon, local_num_lat, num_lev, 1/) ))

      call check(nf90_get_var(VMomDiffncInputID, VMomDiffVarID, dt_v_diff_grid,  &
          start = (/ 1, 1, 1, int(nsteps)/),                               &
          count = (/ local_num_lon, local_num_lat, num_lev, 1/) ))

      call check(nf90_get_var(UMomDissncInputID, UMomDissVarID, dt_u_diss_grid,  &
          start = (/ 1, 1, 1, int(nsteps)/),                               &
          count = (/ local_num_lon, local_num_lat, num_lev, 1/) ))

      call check(nf90_get_var(VMomDissncInputID, VMomDissVarID, dt_v_diss_grid,  &
          start = (/ 1, 1, 1, int(nsteps)/),                               &
          count = (/ local_num_lon, local_num_lat, num_lev, 1/) ))

      call check(nf90_get_var(DiabDissncInputID, DiabDissVarID, dt_temp_diss_grid,  &
          start = (/ 1, 1, 1, int(nsteps)/),                               &
          count = (/ local_num_lon, local_num_lat, num_lev, 1/) ))
