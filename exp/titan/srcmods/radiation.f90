...
! Modified by LJJ 02/2014 for Titan

! module variables
logical :: initialized =.false.
real    :: secs_per_day    = 86400.
real    :: solar_constant  = 15.04 ! W/m^2

real    :: lw_tau0         = 10.0
real    :: lw_tau_exponent = 2.     ! exponent for nonlin. term in lw optital depth
real    :: lw_linear_frac  = 0.0    ! fraction of linear lw absorption
real    :: sw_tau0         = 5.0
real    :: sw_tau_exponent = 0.22   ! exponent in sw optical depth - pressure relation

real    :: single_albedo   = 0.95   ! single scattering albedo
real    :: asymmetry_factor= 0.65
real    :: albedo_sfc      = 0.3 

real    :: day             = 0.0

! modif omp: winter/summer hemisphere
real    :: del_sol         = 1.4   ! Not used for Titan. only used if perpetual_equinox=.true.
!real    :: lw_tau0_eqtr    = 6.0   ! Not used for Titan.
!real    :: lw_tau0_pole    = 1.5   ! Not used for Titan.
!real    :: atm_abs         = 0.2   ! Not used for Titan.
!real    :: sw_diff         = 0.0   ! Not used for Titan.

logical :: fixed_day          = .false.
real    :: fixed_day_value    = 0.       ! time in days from spring equinox 
real    :: days_in_year       = 10758.  ! how many model days in solar year for Titan
...
real    :: orb_ecc         = 0.05415    ! eccentricity of Titan's orbit (unitless) [0.017]
real    :: orb_obl         = 26.7    ! obliquity of Titan's orbit (degrees) [23.45]
real    :: orb_long_perh   = 277.7    ! angle in Titan's orbit between equinox and perihelion (degrees) [281.4]
! end orbital forcing

real, allocatable, dimension(:,:)   :: insolation, h0, h0_arg
...
! Define parameters related to scattering

real, save :: Ga, albedo_inf, albedo_mod, intensity_inflation 


namelist/radiation_nml/ solar_constant, lw_tau0, lw_tau_exponent, lw_linear_frac, &
                        sw_tau0, sw_tau_exponent, single_albedo, asymmetry_factor, &
                        albedo_sfc, del_sol,   &
...
albedo_inf  = ( sqrt(1. - asymmetry_factor*single_albedo) - sqrt(1. - single_albedo) )        &
           / ( sqrt(1. - asymmetry_factor*single_albedo) + sqrt(1. - single_albedo) );

! End modification

!auxiliary quantities in computation of solar flux
albedo_mod       = (albedo_sfc - albedo_inf) / (1. - albedo_sfc*albedo_inf)
intensity_inflation       = 1. / (1. + albedo_mod * albedo_inf * exp(-2*Ga*sw_tau0))

! rescaling factor for optical depth due to scattering
Ga          = 2.*sqrt( (1. - single_albedo) * (1. - asymmetry_factor*single_albedo) );
...
! Paul, 2006 ! ONLY FOR EARTH, NOT FOR TITAN
...
! longwave and shortwave optical depth
n = size(t,3)
...
! compute downward shorwave flux, including scattering
do k = 1,n+1
!  sw_down(:,:,k) = insolation(:,:)*exp(-sw_tau(:,:,k))
   sw_down(:,:,k) = insolation(:,:) * intensity_inflation &
                    * (albedo_inf * albedo_mod * exp(Ga * (sw_tau(:,:,k) - 2.*sw_tau0)) &
                       + exp(-Ga * sw_tau(:,:,k)))
end do
...
!   sw_up(:,:,k) = albedo(:,:)*sw_down(:,:,n+1)
  sw_up(:,:,k)   = insolation(:,:) * intensity_inflation          &
                   * (albedo_mod * exp(Ga * (sw_tau(:,:,k) - 2.*sw_tau0)) & 
...
deallocate (insolation, h0, h0_arg, h0_ann, h0_arg_ann) 
