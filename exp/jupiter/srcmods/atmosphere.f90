...
use            constants_mod, only: grav, pi, dens_h2o, cp_air
...
! begin LJJ addition
use     rayleigh_drag_forcing, only: rayleigh_drag_forcing_init,  &
                                    compute_rayleigh_drag_forcing
! end LJJ addition
...
logical :: rayleigh_drag = .true. 
logical :: jupiter_two_stream = .true. 
! end LJJ addition
...
namelist/atmosphere_nml/ turb, ldry_convection, lwet_convection, roughness_heat, two_stream, mixed_layer_bc,               &
                         ...
                         robert_bucket, raw_bucket, max_bucket_depth_land, damping_coeff_bucket, flux_con, diabatic_acce,  &
                         rayleigh_drag, jupiter_two_stream
...
real, allocatable, dimension(:,:,:) ::                                        &
     ...
     diss_heat_ray,        &   ! heat dissipated by Rayleigh drag                         ! added by LJJ 
...
integer ::                                                                    &
     ...
     id_diss_heat_ray,     &   ! dissipated heat from Rayleigh drag         ! added by LJJ
...
allocate(diss_heat_ray(is:ie, js:je, num_levels))         ! added by LJJ
...
if(jupiter_two_stream) call radiation_init(is, ie, js, je, num_levels, get_axis_id(), Time)
...
call rayleigh_drag_forcing_init(get_axis_id(), Time)
axes = get_axis_id()
...
! begin LJJ modification
if (.not.rayleigh_drag) then
call surface_flux(                                                          &
...
! begin LJJ addition: rayleigh_drag in the pbl boundary layer
if(rayleigh_drag) then
   call compute_rayleigh_drag_forcing( 1,                     ie-is+1, &
                                       1,                     je-js+1, &
                                     Time,                    delta_t, &
                   rad_lat_2d(:,:       ),         dt_ug(:,:,:      ), &
                        dt_vg(:,:,:     ),                             &
...
if(jupiter_two_stream) then
   call radiation_up(is, js, Time,                                    &
                      rad_lat_2d(:,:),                                 &
                      p_half(:,:,:),                                   &
                      surf_lw_down(:,:)+net_surf_sw_down(:,:),         &
                      tg(:,:,:,previous),                              &
                      dt_tg(:,:,:))

! add the internal heat flux
dt_tg(:,:,num_levels) = dt_tg(:,:,num_levels)                          &
  + diabatic_acce * grav*flux_con/(cp_air*(p_half(:,:,num_levels+1)    &
  - p_half(:,:,num_levels)))
endif
! end LJJ addition
...
! loop added by fridoo 01 feb 2012, modified by LJJ
   if(.not.mixed_layer_bc .and. .not.tapio_forcing .and. .not.hs .and. .not. rayleigh_drag) then
...
if(jupiter_two_stream) call radiation_end
