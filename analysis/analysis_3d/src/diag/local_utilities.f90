...
!                            --- input arguments ---                           
    ...
    real, dimension(:, :, :), intent(in) ::                                      &
         field_xyz              ! 3-d array to be differentiated

!                            --- output arguments ---                         

    real, dimension(size(field_xyz, 1), size(field_xyz, 2), size(field_xyz, 3)) ::                  &
         mrdnl_gradient_xyz     ! field_yz differentiated WRT latitude

!                            --- local variables ---                           

    real, dimension(size(field_xyz, 2)) :: merid_dist 
    real :: a0, b0, c0
...
!                            --- executable code ---                           
    ...
    do j=2, nj-1
       ...
       c0 = merid_dist(j+1) - merid_dist(j-1)  ! a0 + b0
       ! meridional gradient by centered differences
       mrdnl_gradient_xyz(:, j, :) = b0 / a0 / c0 * field_xyz(:, j+1, :)              &
                   - a0 / b0 / c0 * field_xyz(:,j-1, :)                          &
                   + (a0 - b0) / a0 / b0 * field_xyz(:, j, :)
    enddo
    ! one-sided differences at poles
    mrdnl_gradient_xyz(:, 1, :) =                                                 &
         ( field_xyz(:, 2, :) - field_xyz(:, 1, :) )                                  &
         / ( merid_dist(2) - merid_dist(1) )

    mrdnl_gradient_xyz(:, nj, :) =                                                &
         ( field_xyz(:, nj, :) - field_xyz(:, nj-1, :) )                              &
         / ( merid_dist(nj) - merid_dist(nj-1) )
...
!----------------------------------------------------------------------------- 
  ...
  function divide_by_psfc_3d(field_xyz, psfc)

    ! divide 3D field by mean surface pressure
...
!                            --- output arguments ---                         
    ...
    real, dimension(size(field_xyz, 1), size(field_xyz, 2), size(field_xyz, 3)) ::                  &
         divide_by_psfc_3d     ! field_xyz divided by surface pressure

!OPEN(91,file='/home/yohai/fms/conv_gray_alb/scripts/my_output.txt')
!  WRITE (* ,91), size(field_xyz, 1), size(field_xyz, 2), size(field_xyz, 3)
!WRITE(91,'(14g25.6)'),'yohai', size(field_xyz, 1), size(field_xyz, 2), size(field_xyz, 3)

!                            --- executable code ---                         

    do k=1, size(field_xyz, 3)
       divide_by_psfc_3d(:, :, k) = field_xyz(:, :, k) / psfc
    enddo

  end function divide_by_psfc_3d
...
!-----------------------------------------------------------------------------
  ...
  function multiply_by_psfc_3d(field_xyz, psfc)

    ! multiply 3D field by mean surface pressure
...
!                            --- output arguments ---                         
    ...
    real, dimension(size(field_xyz, 1), size(field_xyz, 2), size(field_xyz, 3)) ::                  &
         multiply_by_psfc_3d      ! field_xyz multiplied by surface pressure

!                            --- executable code ---                         
        
    do k=1, size(field_xyz, 3)
       multiply_by_psfc_3d(:, :, k) = field_xyz(:, :, k) * psfc
    enddo

  end function multiply_by_psfc_3d
...
!-----------------------------------------------------------------------------
  ...
  function sigma_avg(field_xyz, psfc) 
    ! computes weighted meridional average of 3d field in grid representation
...
!     sigma_avg = field_xyz_ps
   
  end function sigma_avg

!-----------------------------------------------------------------------------

  function sigma_avg_3d(field_xyz, psfc) 
    ! computes weighted meridional average of 3d field in grid representation
    ! (averages in sigma-coordinates must be weighted by surface pressure)
...
!                            --- output arguments ---                         
    ...
    real, dimension(size(psfc, 1), size(psfc, 2), size(field_xyz, 3)) ::      &
!    real, dimension(size(field_xyz, 2), size(field_xyz, 3)) ::                &
         sigma_avg_3d             ! weighted meridional average of field_xyz
...
!    sigma_avg = sum(field_xyz_ps, 1) / float( size(field_xyz, 1) )
     sigma_avg_3d = field_xyz_ps
   
  end function sigma_avg_3d

!-----------------------------------------------------------------------------

!function pdf_avg_3d(field_xyz, psfc, field_bin) 
...
!    real, dimension(size(field_xyz, 1), size(field_xyz, 2), size(field_xyz, 3), size(field_bin)) ::         &
...
!        pdf_avg(i,j,k,ibin(1)) = pdf_avg(i,j,k,ibin(1)) + psfc(i,j) 

!        end do
!      end do
!    end do

    ! normalize with the number of bins -1 (multiply)
    ! now including a longitudinal dimension so don't divide by number of longitudes
!    pdf_avg = pdf_avg * float(size(field_bin)-1)

    
!end function pdf_avg_3d

!------------------------------------------------------------------------------

  function pdf_avg(field_xy, field_bin) 
     ! gives increments to a histogram for the 2d field
     ! note that field_bin gives the bin centers

!                            --- input arguments ---                           

    real, dimension(:, :), intent(in) ::                                   &
         field_xy             ! 2d input field
...
!                            --- output arguments ---                         
    ...
    real, dimension(size(field_xy, 1), size(field_xy, 2), size(field_bin)) ::         &
         pdf_avg             ! change in weighted histogram of field_xy
...
!                            --- executable code ---                           
    ...
    ! loop over the grid field
    do i=1, size(field_xy,1)
      do j=1, size(field_xy,2)
         do k=1, size(field_bin)

            ! add 1 to all bins which field exceeds
            if (field_xy(i,j)>field_bin(k)) then
               pdf_avg(i,j,k) = pdf_avg(i,j,k) + 1
            endif
...
!                            --- executable code ---                      
    ...
    do i=1, size(k_sfc, 1)
       do j=1, size(k_sfc, 2)
          sfc_theta_pdf(i, j, k_sfc(i,j)) = sfc_theta_pdf(i, j, k_sfc(i, j)) + 1.
       enddo
    enddo
...
!                            --- executable code ---                          
    ...
    d_dp_pot_temp   = divide_by_psfc_3d(d_dsig_pot_temp, psfc)
...
!------------------------------------------------------------------------------
  ...
  function buoyancy_frequency(p_full, virtual_temp_grid, pot_temp, d_dp_pot_temp)  
    ! computes mean Brunt-Vaeisaelae frequency N in 3d
...
!                            --- local variables ---                           
    ...
    real, dimension(size(res_circ1, 1), size(res_circ1, 2), size(res_circ1, 3)) ::                &
         res_eddy1,                                                           &
         res_eddy2
...
!                            --- executable code ---                           
    ...
    res_eddy1 = 2. * pi * radius / grav *                                     &
         multiply_by_psfc_3d(mrdnl_eddy_hf, ps_grid_avg) /                       &
         d_dsig_pot_temp
    ...
    res_eddy2 = - 2. * pi * radius / grav *                                   &
         multiply_by_psfc_3d(vrtcl_eddy_hf, ps_grid_avg) /                       &
         d_dy_pot_temp
