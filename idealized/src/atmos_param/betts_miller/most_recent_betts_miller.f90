...
!                                        ,tfreeze,  rvgas,    grav
                                        ,rvgas,    grav
  use	 spherical_fourier_mod, only:    get_wts_lat,        get_sin_lat
...
!
  ...
  real :: slope = 1.0     ! slope of the 'dry' adiabat
  namelist /betts_miller_nml/ cbmts, cbmtd, cbmef, Ahlv, Ahls, slope
...
! diagnostic IDs
  integer :: id_tdt, id_qdt, id_ztc, id_dtd, id_dqd, id_dts, id_dqs,        &
       id_dztref,          id_sztref,             id_top,                   &
         id_base,         id_dztref0,         id_dztref1

  real :: missing_value = -1.e10

  
  real tfreeze ! be mindful of this--need to be changed if using water
contains

!#######################################################################
  
  subroutine betts_miller_init(axes, Time, num_lats_in_subwindow,           &
       local_lon_max, local_num_levels)
    
    integer, intent(in) :: axes(4)
    type(time_type), intent(in) :: Time

    integer, intent(in) :: local_num_levels, num_lats_in_subwindow,         &
         local_lon_max

    integer :: ierr, namelist_unit, io
    
!!$    Ahlv  = hlv      ! Alternate latent heat of vaporization (can be 0)
!!$    Ahls  = hls      ! Alternate latent heat of sublimation (can be 0)


    num_levels = local_num_levels
    lon_max = local_lon_max
    
    allocate (        pk(num_levels+1) )
    allocate (	      bk(num_levels+1) )
    allocate (	 bk_full(num_levels  ) )
    allocate (	 wts_lat(num_lats_in_subwindow) )
    allocate (	sin_lat2(num_lats_in_subwindow) )
    
    call get_pk_bk(pk, bk)


    tfreeze = 0.0 ! be mindful of this!
    ...
    call get_sin_lat(sin_lat2)
    sin_lat2 = sin_lat2 * 2.0
...
!       write(*,*) 'ONE'
       call bmadj(                                                          &
	    ...
	    lon_max,	  num_levels,	 num_levels-1,  num_levels+1,       &
            ...
            c5les,	  wts_lat(krow),		sin_lat2(krow),     &
            dcvgr,	  dcvgs,	                dcvmoi,	            &
...
!*/
   SUBROUTINE BMADJ(							 &
     ...
     real, intent(in)    ::   cbmts,     cbmtd,   cbmef,    twodt        &
         ...
         ,c3les,    c4ies,    c4les,     c5les,   pbudw,    ptwomu       
...
!
      ...
      ZBMEF=CBMEF
      ZMINCP=-10./RD
! setting these to one turns off shallow scheme
!!$      zctopsl=1.0
!!$      zctopss=1.0
! default values
      ZCTOPSL=0.725
...
! the dry adiabat is now multiplied by a constant...this MUST be changed
! if running with water....
!!$         ztc(jl,jk)= ztc(JL,JK+1) +                           &
!!$              slope*( ztc(JL,JK+1)*zdpkpk(JL,JK) - ztc(JL,JK+1) )
!!$
! original code: 
...
!
! look above stable layers (TUNNELING):
     if ((ilab(jl,jk).eq.1).and.(itop(jl).eq.nlev)) then
! original code:
...
!      ZETPBL=0.9  ! this is the default
   ...
   10 CONTINUE
!
! we don't want the boundary layer...
...
!!$ *****************
      LOLIFT(JL)=.true. ! be mindful of this change!!!!
...
!
      ISUM=0
      DO 342 JL=1,NDEEP
! only relax in-cloud layers (SDHYBRID):
!!$      if ((icount(jl).eq.0).and.(jk.le.ibased(jl))) then
! original code:
      IF (ICOUNT(JL).EQ.0) THEN
!!$ *****************
         ztref(jl,jk)= ZTREF(JL,JK+1) +                           &
              slope*( ZTREF(JL,JK+1)*ZDPKD(JL,JK) - ZTREF(JL,JK+1) )
...
!
	 ...
	 ZDSP=ZDSPLOW-(ZDSPLOW-ZDSPFR)*(ZPP1D(JL,NUPBL)-ZPP1D(JL,JK))	    &
      ...
      ENDIF
! only adjust in-cloud layer (SDHYBRID)
!!$      lloc(jl,jk)=(jk.ge.itopd(jl)).and.(jk.le.ibased(jl))
! original code:
...
!!$          if(jl.eq.1) &
!!$                 write(*,*)jl,jk,jit,ZTREF(JL,JK),ZDHTOT(JL)/ZDHDT,ZDHDT
...
! this insures that deep convecting points are not 'swapped' for 
! shallow convecting points because there is no precipatation...
      zrain = 1.0
      zsnow = 0.0
