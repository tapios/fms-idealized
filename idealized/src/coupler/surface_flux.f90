...
use       constants_mod, only: cp_air, hlv, stefan, rdgas, rvgas, grav, dens_h2o
...
subroutine surface_flux_1d (                                           &
     ...
     p_surf,    t_surf,     t_ca,      q_surf,    bucket_depth,        &
     depth_change_lh_1d, depth_change_conv_1d, depth_change_cond_1d,   &
     u_surf, v_surf, rough_mom, rough_heat, rough_moist, gust,         &
...
! ============================================================================
  ...
  real, intent(inout), dimension(:) :: bucket_depth    ! added by LJJ
  ...
  real, dimension(size(t_atm)) ::                          &
       ...
       q_atm,    q_surf0,   depth_change_precip   
...
!     thv_surf= t_surf0  ! surface virtual (potential) T -- just for testing tun off the q_surf
  ...
  where (avail)
     ...
     !depth_change_precip = depth_change_conv_1d + depth_change_cond_1d
     ! modify the bucket hydrology 
     !where (flux_q > 0.0 .and. bucket_depth < depth_change_lh_1d - depth_change_precip)
     !      flux_q = (bucket_depth + depth_change_precip) * dens_h2o/dt
     !      depth_change_lh_1d = flux_q * dt / dens_h2o     
     !endwhere
...
subroutine surface_flux_2d (                                           &
     t_atm,     q_atm_in,   u_atm,     v_atm,     p_atm,     z_atm,    &
     p_surf,    t_surf,     t_ca,      q_surf,    bucket_depth,        &   ! added by LJJ
     depth_change_lh, depth_change_conv, depth_change_cond,            &   ! added by LJJ      
     u_surf,    v_surf,                                                &   
     rough_mom, rough_heat, rough_moist, gust,                         &
     flux_t,    flux_q,     flux_r,    flux_u,    flux_v,              &
     cd_m,      cd_t,       cd_q,                                      &
  ...
  real, intent(inout), dimension(:,:) :: q_surf
  real, intent(inout), dimension(:,:) :: bucket_depth ! added by LJJ
  real, intent(inout), dimension(:,:) :: depth_change_lh ! added by LJJ
  real, intent(in), dimension(:,:)    :: depth_change_conv, depth_change_cond ! added by LJJ
  real, intent(in) :: dt
  ...
  do j = 1, size(t_atm,2)
     call surface_flux_1d (                                           &
          ...
          p_surf(:,j),    t_surf(:,j),     t_ca(:,j),      q_surf(:,j),    bucket_depth(:,j),             &
          depth_change_lh(:,j), depth_change_conv(:,j), depth_change_cond(:,j),                           &
          u_surf(:,j),    v_surf(:,j),  rough_mom(:,j), rough_heat(:,j), rough_moist(:,j), gust(:,j),     &
