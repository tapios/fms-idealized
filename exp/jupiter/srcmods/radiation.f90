...
real    :: solar_constant  = 50.7 ! W/m^2
real    :: day             = 0.0
! modif omp: winter/summer hemisphere
real    :: lw_tau0     = 50.0
real    :: sw_tau0     = 3.0
real    :: lw_tau_exponent  = 2.0 
real    :: sw_tau_exponent  = 1.0 
real    :: single_albedo    = 0.8   ! single scattering albedo
real    :: back_scatter     = 0.398 ! back scattering factor
...
logical :: uniform_insolation = .false. 
logical :: scaled_insolation  = .false. 
real    :: factor_mean        = 0.0
real    :: factor_vari        = 0.0
...
real, save :: albedo, Ga, g

namelist/radiation_nml/ solar_constant, lw_tau0, sw_tau0, lw_tau_exponent, sw_tau_exponent, &
                        single_albedo, back_scatter, uniform_insolation, scaled_insolation, &
                        factor_mean, factor_vari, diabatic_acce,                            &
...
! parameters in calculation of solar flux
g          = 1 - 2.*back_scatter;                 ! asymmetry factor
albedo     = ( sqrt(1. - g*single_albedo) - sqrt(1. - single_albedo) )        &
           / ( sqrt(1. - g*single_albedo) + sqrt(1. - single_albedo) );
Ga         = 2.*sqrt( (1. - single_albedo) * (1. - g*single_albedo) );
...
!########################################################################!
! IDEALIZED POLYNOMIAL REPRESENTATION FOR EQUINOX INSOLATION THAT APPROXIMATES JUPITER INSOLATION IN THE ANNUAL-MEAN
!########################################################################!
...
! UNIFORM INSOLATION
!########################################################################!
else if ( uniform_insolation ) then
! liu, 2009
  insolation = solar_constant/4. 

!########################################################################!
! SCALED INSOLATION
!########################################################################!
else if ( scaled_insolation ) then
! liu, 2011
  insolation = solar_constant*(factor_mean + factor_vari/pi * cos(lat(:,:)))   
...
! longwave and shorwave optical depths
n = size(t,3)

do k = 1, n+1
   lw_tau(:,:,k)  = lw_tau0 * (p_half(:,:,k)/pstd_mks )**lw_tau_exponent
...
! compute downward longwave flux
lw_down(:,:,1) = 0.0
do k = 1,n
    lw_down(:,:,k+1) = lw_down(:,:,k)*lw_dtrans(:,:,k) + b(:,:,k)*(1.0 - lw_dtrans(:,:,k))
end do

! computer downward shortwave flux
do k = 1,n+1
   sw_down(:,:,k) = insolation(:,:)*(1-albedo)*exp(-Ga * sw_tau(:,:,k))
end do

surf_lw_down     = lw_down(:,:,n+1)
sw_down_toa      = sw_down(:,:,1)
net_surf_sw_down = sw_down(:,:,n+1)
...
subroutine radiation_up (is, js, Time_diag, lat, p_half, b_surf, t, tdt)
...
type(time_type), intent(in)         :: Time_diag
real, intent(in) , dimension(:,:)   :: lat
real, intent(in) , dimension(:,:)   :: b_surf
real, intent(in) , dimension(:,:,:) :: t, p_half
...
deallocate (b_surf, net_lw_toa, net_lw_surf, sw_down_toa)
