...
! ----- timing flags -----
   ...
   logical use_namelist ! added by CW:
...
!-----------------------------------------------------------------------
      ...
      integer :: dt_atmos = 0
      logical :: override       ! added by CW
      
      namelist /main_nml/ current_time, dt_atmos,  &
                          days, hours, minutes, seconds, override
...
!----- read restart file -----
   ...
   if (file_exist('INPUT/atmos_model.res')) then
       ...
       read  (unit,*) date
       call mpp_close (unit)
       use_namelist = .false. ! added by CW
   else
      use_namelist = .true. ! added by CW
! removed by CW:
...
!----- write restart file ------
      ...
      if ( mpp_pe() == mpp_root_pe() ) then
           call mpp_open (unit, 'RESTART/atmos_model.res', form=MPP_ASCII, action=MPP_OVERWR, &
                          access=MPP_SEQUENTIAL, threading=MPP_SINGLE, nohdrs=.true. )
           write (unit,'(6i9,8x,a)') date, &               ! LJJ modification for large number of days
